            ...
            <div class="col-md-6 col-12">
                <form method="post">
                    <div class="form-floating mb-3">
                        <input type="text" class="form-control rounded-4" id="captionFormControlInput" name="caption"
                               placeholder="Katt i Maxikasse..." value="${image.caption!""}">
                ...
                <hr/>
                <div>
                    <button class="btn btn-outline-danger" id="deleteButton">Radera bild <i class="bi-trash3 ms-1"></i>
                    </button>
                    <form id="deletionChallenge"
                          action="/image/${image.id}/delete<#if redirect??>?redirect=${redirect}</#if>" method="post"
                          hidden>
                        <#if image.variants?size == 0>
                        <#else>
                            <p class="fw-bold">Bilden
                                har ${image.variants?size} <#if image.variants?size == 1>variant<#else>varianter</#if>
                                som också kommer att raderas. För att undvika att en variant raderas, öppna denna och
                                klicka på &quot;frikoppla&quot;.</p>
                        </#if>

                        <p>För att kunna radera bilden måste du skriva &quot;<strong>radera bild</strong>&quot; i rutan
                            nedan.</p>
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control rounded-4" id="deleteChallengeConfirmation"
                                   placeholder="Radera bild">
                            <label for="deleteChallengeConfirmation">Bekräftelse</label>
                        </div>
                        <button class="btn btn-danger btn-lg w-100 rounded-4" id="confirmDeleteButton" type="submit"
                                disabled>Ja, jag vill radera bilden <strong>permanent</strong>.
                        </button>
                    </form>
                </div>
            </div>
